...
#ifndef RAY_TRACER_21_CAMERA_HPP
#define RAY_TRACER_21_CAMERA_HPP
...
class Camera {
 public:
  explicit Camera(CameraParameters<CoordType> parameters) : parameters_(parameters) {
    parameters_.direction = math3d::Norm(parameters_.direction);
    parameters_.up_direction = math3d::Norm(parameters_.up_direction);
    right_direction_ = math3d::Norm(parameters_.direction % parameters_.up_direction);
  }

  image::IImagePtr TakePhoto(image::IImagePtr film, size_t chunk_w = 100, size_t chunk_h = 100, size_t n_workers = 2) {
    ...
    for (size_t chunk_y = 0; chunk_y < film->Height(); chunk_y += chunk_h) {
      for (size_t chunk_x = 0; chunk_x < film->Width(); chunk_x += chunk_w) {
        counter.AddChunk();
        thread_pool->SubmitTask([chunk_x, chunk_y, chunk_w, chunk_h, &film, &counter, this]() {
          for (size_t x = chunk_x; x < std::min(chunk_x + chunk_w, film->Width()); ++x) {
            for (size_t y = chunk_y; y < std::min(chunk_y + chunk_h, film->Height()); ++y) {
 ...
 private:
  void ProcessPixel(image::IImagePtr film, size_t x, size_t y) {
    ...
    math3d::Vector3D<CoordType> ray_direction =
        cos(y_angle) *
            (cos(x_angle) * parameters_.direction + sin(x_angle) * right_direction_) +
...
#endif  // RAY_TRACER_21_CAMERA_HPP
